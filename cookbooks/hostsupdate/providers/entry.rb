...
action :create do
  if hostsupdate.contains?(new_resource)
...
# one will be created
action :append do
  unless hostsupdate.contains?(new_resource)
...
action :remove do
  if hostsupdate.contains?(new_resource)
    converge_by("Remove #{new_resource}") do
      hostsupdate.remove(new_resource.ip_address)
      hostsupdate.save
    end
  else
...
# The hostsupdate object
#
# @return [Manipulator]
#   the manipulator for this hostsupdate
def hostsupdate
  @hostsupdate ||= Manipulator.new(node)
end

...
describe Manipulator do
  ...
  describe '.initialize' do
    ...
    it 'raises a fatal error if the hostsupdate does not exist' do
  ...
  describe '#hostsupdate_path' do
    before do
      manipulator.class.send(:public, :hostsupdate_path)
      File.stub(:exists?).and_return(true)
    end

    context 'with no node attribute specified' do
      it 'returns /etc/hosts on a *nix machine' do
        expect(manipulator.hostsupdate_path).to eq('/etc/hosts')
      end
      it 'returns C:\Windows\system32\drivers\etc\hosts on a Windows machine' do
        ...
        expect(Manipulator.new(windows_attributes).hostsupdate_path).to eq('C:\Windows\system32\drivers\etc\hosts')
      end
    end

    context 'with a custom hostsupdate node attribute' do
      it 'returns the custom path' do
        custom_path = '/custom/path'
        expect(Manipulator.new(node.merge({'hostsupdate' => { 'path' => custom_path } })).hostsupdate_path).to eq(custom_path)
      end
    end
  end

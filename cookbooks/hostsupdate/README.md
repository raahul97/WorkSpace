hostsupdate LWRP
==============
[![Build Status](https://travis-ci.org/customink-webops/hostsupdate.png?branch=master)](https://travis-ci.org/customink-webops/hostsupdate)

`hostsupdate` provides an LWRP for managing your `/etc/hosts` (or Windows equivalent) file using Chef.
...
Not specifying the `unique` parameter will result in duplicate hostsupdate entries.

#### `create`
Creates a new hosts file entry. If an entry already exists, it will be overwritten by this one.

```ruby
hostsupdate_entry '1.2.3.4' do
...
#### `append`
Append a hostname or alias to an existing record. If the given IP address doesn't not already exist in the hostsupdate, this method behaves the same as create. Otherwise, it will append the additional hostname and aliases to the existing entry.
...
Usage
-----
If you're using [Berkshelf](http://berkshelf.com/), just add `hostsupdate` to your `Berksfile`:

```ruby
cookbook 'hostsupdate'
```

Otherwise, install the cookbook from the community site:

    knife cookbook site install hostsupdate

Have any other cookbooks *depend* on hostsupdate by editing editing the `metadata.rb` for your cookbook.
...
Note that you can specify a custom path to your hosts file in the `['hostsupdate']['path']` node attribute. Otherwise, it defaults to sensible paths depending on your OS.

### Testing
If you are using [ChefSpec](https://github.com/sethvargo/chefspec) to unit test a cookbook that implements the `hostsupdate_entry` LWRP, this cookbook packages customer matchers that you can use in your unit tests:

- `append_hostsupdate_entry`
- `create_hostsupdate_entry`
- `create_hostsupdate_entry_if_missing`
- `remove_hostsupdate_entry`
- `update_hostsupdate_entry`

For example:

```ruby
it 'creates a hostsupdate entry for the DNS server' do
  expect(chef_run).to create_hostsupdate_entry('1.2.3.4')

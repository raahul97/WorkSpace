...
class Manipulator
  ...
  # hostsupdate is not found, an exception is raised.
  #
  ...
  def initialize(node)
    ...
    # Fail if no hostsupdate is found
    unless ::File.exists?(hostsupdate_path)
      raise RuntimeError, "No hostsupdate exists at `#{hostsupdate_path}'!"
    end

    @entries = []
    collect_and_flatten(::File.readlines(hostsupdate_path))
  end

  # Return a list of all IP Addresses for this hostsupdate.
  #
  # @return [Array<IPAddr>]
  ...
  # Add a new record to the hostsupdate.
  #
  # @param [Hash] options
  ...
  # Save the new hostsupdate to the target machine. This method will only write the
  # hostsupdate if the current version has changed. In other words, it is convergent.
  def save
    entries = []
    entries << '#'
    entries << '# This file is managed by Chef, using the hostsupdate cookbook.'
    ...
    entries << '# hostsupdate will be unable to guarantee relative priority in'
    ...
    file = Chef::Resource::File.new(hostsupdate_path, node.run_context)
  ...
  # Determine if the current hostsupdate contains the given resource. This
  ...
  # The path to the current hostsupdate.
  #
  # @return [String]
  #   the full path to the hostsupdate, depending on the operating system
  #   can also be overriden in the node attributes
  def hostsupdate_path
    return @hostsupdate_path if @hostsupdate_path
    @hostsupdate_path = node['hostsupdate']['path'] || case node['platform_family']
                                                   when 'windows'
  ...
  # The current sha of the system hostsupdate.
  #
  # @return [String]
  #   the sha of the current hostsupdate
  def current_sha
    @current_sha ||= Digest::SHA512.hexdigest(File.read(hostsupdate_path))
  end
